...
def is_key_vault_runtime_disabled() -> bool:
    return _is_truthy(os.getenv("PRINCETON_SENTINEL_DISABLE_KEY_VAULT_RUNTIME"))


def is_azure_key_vault_runtime_enabled() -> bool:
    return (
        bool(key_vault_env.normalize_vault_url(os.getenv("AZ_KEY_VAULT_URL")))
        and not is_local_docker_deployment()
        and not is_key_vault_runtime_disabled()
    )
...
def is_required_runtime_manifest_key(name: str) -> bool:
    return bool((_runtime_keys().get(name) or {}).get("required"))
...
def get_runtime_env(name: str, default: str | None = None) -> str | None:
    ...
    try:
        ...
        if stale:
            ...
            return stale
        if not is_required_runtime_manifest_key(name):
            return default
        raise
