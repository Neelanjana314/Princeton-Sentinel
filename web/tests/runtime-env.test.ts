...
function resetEnv() {
  ...
  delete process.env.GRAPH_FETCH_TIMEOUT_MS;
...
test("live runtime env bypasses Key Vault when runtime is disabled", async () => {
  resetEnv();
  process.env.AZ_KEY_VAULT_URL = "https://vault.example";
...
test("optional live runtime env uses caller fallback after Key Vault failure", async () => {
  resetEnv();
  process.env.AZ_KEY_VAULT_URL = "https://vault.example";
  runtimeEnv.setRuntimeEnvTokenProviderForTests(async () => "token");
  runtimeEnv.setRuntimeEnvFetchForTests((async () => {
  ...
  assert.equal(await runtimeEnv.getPositiveIntRuntimeEnv("GRAPH_FETCH_TIMEOUT_MS", 15000), 15000);
  resetEnv();
});
