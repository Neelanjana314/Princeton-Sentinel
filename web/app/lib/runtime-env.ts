...
function isKeyVaultRuntimeDisabled() {
  return isTruthy(process.env.PRINCETON_SENTINEL_DISABLE_KEY_VAULT_RUNTIME);
}
...
export function isAzureKeyVaultRuntimeEnabled() {
  return (
    Boolean(normalizeVaultUrl(process.env.AZ_KEY_VAULT_URL)) &&
...
function isRequiredRuntimeManifestKey(name: string) {
  return Boolean(getRuntimeKeys().get(name)?.required);
}
...
export async function getRuntimeEnv(name: string): Promise<string | undefined> {
  ...
  try {
    ...
    if (!isRequiredRuntimeManifestKey(name)) {
      return undefined;
    }
    throw error;
  }
